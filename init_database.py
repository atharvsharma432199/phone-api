...
# Direct file ID - config import se avoid karte hain
GOOGLE_DRIVE_FILE_ID = "1C726IZypnLOY85daHXiGQv4c7kfoXHOT"  # YAHAN DIRECT FILE ID DAALO
...
def main():
    ...
    if os.path.exists('phone_data.db'):
        ...
        # For Render, always reinitialize if file exists but might be empty
        conn = sqlite3.connect('phone_data.db')
        cursor = conn.cursor()
        try:
            cursor.execute('SELECT COUNT(*) FROM phone_data')
            ...
            if count > 0:
                print(f"✅ Database already has {count} records. Skipping initialization.")
                conn.close()
                return True
        except:
            pass
        conn.close()
        print("🔄 Database exists but may be empty. Reinitializing...")
